import type { Debt, PayoffPlan, PayoffScheduleMonth, BalanceTransferOption, ConsolidationOption, ScoreSimulation, CashFlowImpact } from "@shared/schema";
...
export function calculatePayoffPlan(
  ...
  const payoffDate = schedule.length > 0 ? schedule[schedule.length - 1].date : new Date().toISOString().slice(0, 7);
...
const BALANCE_TRANSFER_OFFERS: Omit<BalanceTransferOption, "estimatedSavings" | "monthlyPayment" | "totalCost">[] = [
...
export function screenBalanceTransfers(
  creditCardDebts: Debt[]
...
export function screenConsolidation(debts: Debt[]): ConsolidationOption[] {
  ...
  const currentTotalMinimum = debts.reduce((s, d) => s + d.minimumPayment, 0);
...
export function simulateScoreImpact(debts: Debt[], currentScoreRange: [number, number]): ScoreSimulation {
  ...
  if (utilization > 10 && totalLimit > 0) {
    ...
    const prev = actions.length > 0 ? actions[actions.length - 1].newEstimate : currentScoreRange;
    actions.push({
      action: `Pay down $${paydownNeeded.toFixed(0)} to reach 10% utilization`,
      impact,
      newEstimate: [currentScoreRange[0] + impact[0], Math.min(currentScoreRange[1] + impact[1], 850)],
      timeframe: "1-2 billing cycles",
    });
  }
...
export function calculateUtilization(debts: Debt[]) {
  const ccDebts = debts.filter((d) => d.type === "credit_card");
  const totalLimit = ccDebts.reduce((s, d) => s + (d.creditLimit || 0), 0);
